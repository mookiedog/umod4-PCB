...
### 10K Resistor Substitution

The schematic specifies the sixteen 10K/0603 pullup/pulldown resistors using LCSC basic part C25804.
That specific basic part is prone to going out of stock at JLCPCB/LCSC, even though 10K is otherwise a very common value.

If C25804 is out of stock at order time, it is fine to manually substitute a 12K/0603 basic part in the exported BOM csv before uploading it for fabrication.
All sixteen of these resistors are used purely as pullups/pulldowns, where the exact value is not critical.
A 12K substitution was used for the first production run (as a 0402 part, before the 4V3 pad-shrink rework) with no observed issues.

Do not change the schematic value or LCSC_PART for these parts to 12K.
Leave the schematic specifying the intended 10K/C25804, and only patch the LCSC Part # column in the generated BOM csv if the 10K basic part is unavailable when you go to order.
Since the footprint changed from 0402 to 0603 as part of 4V3, look up a current in-stock 12K/0603 basic part at order time rather than reusing the old 0402-package substitute part number.
